...
def create(db: Session, sandwich):
    ...
    db_sandwich = sandwich_model.Sandwich(**sandwich.model_dump())
...
def read_all(db: Session):
    return db.query(sandwich_model.Sandwich).all()


def read_one(db: Session, sandwich_id):
    return db.query(sandwich_model.Sandwich).filter(sandwich_model.Sandwich.id == sandwich_id).first()


def update(db: Session, sandwich_id, sandwich):
    # Query the database for the specific sandwich to update
    db_sandwich = db.query(sandwich_model.Sandwich).filter(sandwich_model.Sandwich.id == sandwich_id)
